...
export default class Room extends EventEmitter {
  ...
  join() {
    ...
    const wsTransport = new WebSocketTransport(`wss://localhost:2345`);
  ...
  async _prepareSendTransport(device) {
    const transportInfo = await this.peer
      ...
      console.error(transportInfo);
      
    // var iceUrl = 'stun:live.taoqiu.top:'+transportInfo.iceCandidates[0].port;
    // transportInfo.iceServers = [{ urls: iceUrl }];
